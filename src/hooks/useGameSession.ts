...
export function useGameSession({ navigate }: UseGameSessionOptions) {
  ...
  const [gameTab, setGameTab] = useState("new");
  const [gameMatchKey, setGameMatchKey] = useState(0);
  const [postSaveRematch, setPostSaveRematch] = useState<unknown>(null);
  const [linkedPlayers, setLinkedPlayers] = useState<Record<string, LinkedPlayerState[]>>({});
  ...
  const handleRematchRequest = useCallback(({
    ...
    playerNames?: string[];
    linkedPlayers?: LinkedPlayerState[];
    ...
    let nextLinkedPlayers: LinkedPlayerState[] = [];
    ...
    if (game?.type === "chin" || game?.type === "ajedrez" || game?.type === "esquinados") {
      ...
      nextLinkedPlayers = previousLinkedPlayers
        ...
        .filter(Boolean) as LinkedPlayerState[];
    } else {
